...
describe Spree::CustomizationOption, type: :model do
  let(:customization_option) { create(:customization_option) }
  let(:order) { create(:order) }
  let!(:line_item) { create(:line_item, order: order) }
  let(:customizations) {create(:customizations, line_item: line_item)}
